...
import com.iyuba.concept2.protocol.UserEngLevelRequest;
import com.iyuba.concept2.protocol.UserEngLevelResponse;
...
public class IntelLearningActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		GitHubImageLoader.Instace(mContext).setCirclePic(String.valueOf(UserInfoManager.getInstance().getUserId()), img);
		...
		commit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				String gtalkLevel = String.valueOf(findString((String) lslevel
						.getText()));
				if (plevel.isEmpty() || pls.isEmpty() || prw.isEmpty()
						|| glevel.isEmpty() || greadLevel.isEmpty()
						|| gtalkLevel.isEmpty())
					Toast.makeText(mContext, "请完善英语学习相关信息", Toast.LENGTH_LONG).show();
				else {
					try {
						Toast.makeText(mContext, "正在上传...", Toast.LENGTH_SHORT).show();
						ClientSession.Instace().asynGetResponse(
								new UserEngLevelRequest(String.valueOf(UserInfoManager.getInstance().getUserId()), plevel, prw,
