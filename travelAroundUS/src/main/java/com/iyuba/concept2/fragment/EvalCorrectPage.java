 ...
 */
public class EvalCorrectPage extends DialogFragment {
    ...
    //单词评测数据
    private EvaluationBeanNew evaluateBean;
    ...
    //设置单词数据
    public void setEvaluateBean(EvaluationBeanNew bean) {
        this.evaluateBean = bean;

        //获取第一个数据显示
        if (bean != null && bean.getWords() != null && bean.getWords().size() > 0) {
            int flag = 0;
            for (int i = 0; i < bean.getWords().size(); i++) {
                double score = Double.parseDouble(bean.getWords().get(i).getScore());
                if (flag == 0 && (score < errorWordScore || score > rightWordScore)) {
                    flag++;
                    titleWord = bean.getWords().get(i).getContent().replaceAll("[?:!.,;\"]*([a-zA-Z]+)[?:!.,;\"]*", "$1");
                    wordScore = score;
                    wordId = Integer.parseInt(bean.getWords().get(i).getIndex());

                    userPorn = bean.getWords().get(i).getUser_pron2();
                    wordPorn = bean.getWords().get(i).getPron2();
                }
            }
        }
    }
    ...
    private void showData() {
        if (evaluateBean != null
                && evaluateBean.getWords() != null
                && evaluateBean.getWords().size() > 0) {

            int clickCount = 0;
            for (int i = 0; i < evaluateBean.getWords().size(); i++) {
                EvaluationBeanNew.WordsBean wordsBean = evaluateBean.getWords().get(i);
                if (wordsBean != null
...
//            if (clickCount==1){
//                tvSentence.setText(ResultParse.setSenResultEval(evaluateBean.getWords()));
//            }else
            if (clickCount > 0) {
                ...
                for (int i = 0; i < evaluateBean.getWords().size(); i++) {
                    EvaluationBeanNew.WordsBean tempWord = evaluateBean.getWords().get(i);
                    int curIndex = i;
