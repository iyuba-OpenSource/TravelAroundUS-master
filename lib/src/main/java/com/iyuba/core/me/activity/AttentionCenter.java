...
public class AttentionCenter extends BasisActivity implements
	...
	public static void start(Context context,int userId){
		...
		intent.setClass(context,AttentionCenter.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//currUserid = AccountManager.Instance(mContext).userId;;
		initWidget();
	...
	private void initFansView() {
		...
		adapter = new AttentionListAdapter(mContext,attentionArrayList);
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 4:
				handler.sendEmptyMessage(3);
				Log.e("TagSize",attentionArrayList.size()+"");
...
//
				startActivity(PersonalHomeActivity.buildIntent (mContext,
						Integer.valueOf(attentionArrayList.get(position).followuid),
